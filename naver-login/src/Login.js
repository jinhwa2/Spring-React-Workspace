...
const Login = () => {
  ...
  useEffect(() => {
    const 유저정보 = () => {
       fetch('htttp://localhost:9010/userInfo') //HTTP METHOD(GET POST DELETE PUT)
       ...
       .catch(err => {  // 위 두 then모두 몬제가 생겼을 때 문제를 catch할 구문
         console.error("Erron user INFO :" , err);
         })
      };
      // 유저정보 기능을 실행하는 구문과
      유저정보() ;
     
    },[]);  //최초 실행인지 주기적으로 계속 실행하는지 효과 설정
