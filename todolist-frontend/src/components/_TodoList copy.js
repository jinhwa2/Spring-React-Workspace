...
const TodoList = () => {
    ...
    // 할 일 추가버튼 기능 설정
     ...
     const 할일수정버튼 = (todo, index) => {
       ...
       fetch("/todo", {
        method : "put", //controller putMappint 작성한다했기에 put
        // headers에서
        // Content-Type는 Controller로 값을 전달할 때 
        // 이 값이 이미지, 동영상, 글자 등 어떤 파일인지 전달하는 공간
        headers : {'Content-Type' : 'application/json'},
        // JSON으로 된 파일을 문자로 변경해서 글자 취급으로 사용
        body: JSON.stringify({
        todoNo: todo.todoNo,
        isDone : todo.isDone === 'O' ? 'X' : 'O' 
         /*
          ...
          할일 완료 여부에 O로 표시되어 있으면 X로 글자 변경
                     ...
                     todo.isDone === 'O' : 'O' 
          할일 완료 여부에 X로 표시되어 있으면 O로 글자 변경
          */
        })
       })
       .then(response => response.text())
     ...
     const 할일삭제버튼 = (todoNo,index) => {
       ...
       .then(result => {
          ...
          /*
            ...
            새로운목록리스트.괄호 안에 작성한 부분 제외하고 목록 새로작성(내가 선택한번호,하나만 삭제);
