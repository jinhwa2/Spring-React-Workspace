...
public class PostSeviceImpl implements PostService {
	...
	public void uploadImages(MultipartFile[] files, String title, String content) {
		...
		File uploadeDirFile = new File(uploadDir);
		// 만약에 폴더가 존재할 경우
		//upodaDirFile.exists() = 폴더가 존재한다.
		// !upodaDirFile.exists() = 폴더가 존재하지 않는다.
		if(!uploadeDirFile.exists()) {
			System.out.println("폴더가 존재하지 않아 폴더를 생성합니다.");
			if (!uploadeDirFile.mkdir()) {
		...
		try {
		   ...
		   // 한번더 리스트로 목록 변환
			fileNames = List.of(files).stream().map(file -> {
				...
				// 랜덤으로 작성한 이름 _원본이름_로 랜덤이름과 원본이름 구분짓고 싶을 때
