...
public class PaymentController {
      //application.properties에서 설정 키이름을 가져오기 위해 value
	...
	//widget 이르는 주소로 결제정보가 들어오면 결제확인창구로 넘겨주는 것 결제정보와 결제하고자 하는 사용자의 비밀번호
	...
	//payment 라는 주소로 결제 정보가 들어오면 결제확인 창구로 넘겨주는 것 결제정보와 결제하고자 하는 사용자의 비밀번호
	@PostMapping("/payment")
	...
	private ResponseEntity<?> confirmPayment(Map<String,String> requestBody,String encodedKey){
	 //fetch("https://api.tosspayments.com/v1/payments/confirm", {	
		...
		headers.set("Authorization", encodedKey);//encryptedApiSecretKey을 위해서 encodedKey 작성해줬음
...
/*****
 ...
 헤더(headers) : HTTP 헤더 정보를 포함 ex) 글자인지 이미지인지 동영상인지.. 어떤 파일이 들어오는 것인가? 누가 보내나?
